...
const AuthPage: React.FC = () => {
  ...
  // --- X (Twitter) Login - THE FIX ---
  const handleXLogin = async (): Promise<void> => {
    const { error } = await supabase.auth.signInWithOAuth({
      // Use 'twitter' here to resolve the 400 "Unsupported provider" error.
      provider: 'twitter', 
      options: {
        redirectTo: REDIRECT_URL,
        queryParams: {
          // Standard scopes for OAuth 2.0; removing 'offline.access' for stability.
          scope: 'users.read tweet.read',
        },
      },
    });
    // This logs the error to help with further debugging if needed.
