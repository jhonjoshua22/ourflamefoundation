...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] bg-[#0a0a0a]/90 backdrop-blur-md text-white border-b border-white/10 transition-all duration-300 font-sans">
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center gap-3 shrink-0" onClick={() => setIsOpen(false)}>
            ...
            <h2 className="text-2xl lg:text-3xl font-black tracking-tighter text-white uppercase italic leading-none hidden sm:block">
              Our <span className="text-orange-600">Flame</span> <br /> 
              <span className="text-lg lg:text-xl not-italic font-light tracking-[0.2em]">Foundation</span>
            </h2>
          </Link>
          ...
          <div className="hidden xl:flex items-center gap-4">
            <div className="flex items-center gap-5 mr-4">
              {navLinks.map((link) => (
                <a 
                  key={link.name} 
                  href={link.href} 
                  className="text-[10px] font-black uppercase tracking-[0.15em] text-gray-400 hover:text-white transition-colors whitespace-nowrap"
                >
                  {link.name}
                </a>
              ))}
            ...
            <button 
              ...
              className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white transition-all rounded-none"
            >
              {isDark ? <Sun className="w-3.5 h-3.5 text-yellow-500" /> : <Moon className="w-3.5 h-3.5 text-gray-400" />}
            </button>
            ...
            {user ? (
              ...
              <Link to="/login" className="text-[10px] font-black uppercase tracking-widest text-white hover:text-orange-600 transition-colors flex items-center gap-2 px-2">
            ...
            <a 
              href="#footer" 
              className="bg-orange-600 px-5 py-2.5 rounded-none font-black text-[10px] uppercase tracking-[0.2em] text-white hover:bg-orange-500 transition-all active:scale-95 whitespace-nowrap"
            >
              Get In Touch
            </a>
          </div>
      ...
      {/* MOBILE MENU */}
      <div className={`xl:hidden fixed inset-x-0 bottom-0 top-20 z-[99] bg-[#0a0a0a] border-t border-white/10 transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
          ...
          <div className="flex flex-col gap-4">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className="text-xl font-black uppercase italic tracking-tighter text-white hover:text-orange-600 transition-colors border-b border-white/5 pb-2" 
